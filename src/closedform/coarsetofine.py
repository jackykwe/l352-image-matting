...
from . import explicit, updownsampling
...
def get_linear_coefficients(alpha, I, epsilon, window_size, *, debug_levels_count):
    ...
    for i in tqdm(
        ...
        desc=f"Getting linear coefficients for upsampling to layer {debug_levels_count}",
...
def upsample_alpha_using_image(downsampled_alpha, downsampled_I, I, epsilon, window_size, *, debug_levels_count):
    """
    ...
    assert result.dtype == float
    logging.debug("upsample_alpha_using_image returning")
    return result
...
def solve_alpha_coarse_to_fine(
    ...
    logging.debug(f"[fineness {levels_count}] solve_alpha_coarse_to_fine invoked")
    ...
    if levels_count >= 2:
        ...
        # logging.debug(f"[fineness {levels_count}] displaying downsampled_alpha")
        ...
        logging.debug(f"  [fineness {levels_count}]STATS np.mean(alpha) = {np.mean(alpha)}")
        logging.debug(f"  [fineness {levels_count}]STATS np.min(alpha) = {np.min(alpha)}")
        logging.debug(f"  [fineness {levels_count}]STATS np.max(alpha) = {np.max(alpha)}")

        # logging.debug(f"[fineness {levels_count}] displaying (upsampled) alpha")
        # logging.debug(f"[fineness {levels_count}] np.min(alpha) = {np.min(alpha)}")
        # logging.debug(f"[fineness {levels_count}] np.max(alpha) = {np.max(alpha)}")
    ...
    if levels_count <= explicit_alpha_levels_count:
        logging.debug(f"[fineness {levels_count}] EXPLICIT RUNNING")
    ...
    logging.debug(f"    [fineness {levels_count}] solve_alpha_coarse_to_fine returning alpha... (displaying)")
    logging.debug(f"    [fineness {levels_count}] np.mean(alpha) = {np.mean(alpha)}")
    logging.debug(f"    [fineness {levels_count}] np.min(alpha) = {np.min(alpha)}")
    logging.debug(f"    [fineness {levels_count}] np.max(alpha) = {np.max(alpha)}")
    # skimage.io.imshow(alpha)
    # skimage.io.show()
    logging.debug(f"    [fineness {levels_count}] solve_alpha_coarse_to_fine returned")
