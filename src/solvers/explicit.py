...
def get_laplacian(I, constrained_map, epsilon, window_size):
    ...
    constructor_row_indices = np.zeros((constructor_length, 1))
    constructor_col_indices = np.zeros((constructor_length, 1))
    constructor_vals = np.zeros((constructor_length, 1))
    ...
    for i in range(window_size, H - window_size):
        for j in range(window_size, W - window_size):
            ...
            constructor_row_indices[len:len + neighbourhood_size_squared] = npmatlib.repmat(window_indices, neighbourhood_size, 1).reshape(neighbourhood_size_squared, 1, order="F")
            constructor_col_indices[len:len + neighbourhood_size_squared] = npmatlib.repmat(window_indices, neighbourhood_size, 1).reshape(neighbourhood_size_squared, 1)
    ...
    W = spsparse.csr_array((constructor_vals, (constructor_row_indices, constructor_col_indices)), shape=(H * W, H * W)) # this is A in MATLAB code
    W_row_sum = W.sum(axis=1)  # this is sumA in MATLAB code
    D = spsparse.diags(W_row_sum, 0, shape=(H * W, H * W), format="csr")
    return D - W
...
def solve_alpha_explicit(I, constrained_map, constrained_vals, epsilon, window_size):
    ...
    b = (constrained_map * constrained_vals).flatten()
    alpha = cholesky(L + lagrangian_lambda * D)(lagrangian_lambda * b)  # HW x 1 vector
